document.addEventListener('DOMContentLoaded', async function () {
    ...
    const loadingIndicator = document.getElementById('loading_20');
    ...
    urlTsidTw = `${setBaseUrl}timeseries/group/TW-DO?office=${office}&category-id=${lake}`;
    ...
    const fetchTsidData = async () => {
        try {
            ...
            async function loginStateController() {
                cdaSaveBtn = document.getElementById("cda-btn-do"); // Get the button by its ID
            ...
            let formattedTwData = hourlyTwData.map(entry => {
            ...
            function createTable(formattedTwData, tsidTw) {
                ...
                table.id = "do";
                table.style.width = "100%"; // Make table fill full width
                ...
                const output20Div = document.getElementById("output20");
                output20Div.innerHTML = "";
                output20Div.appendChild(table);
                ...
                parameterRow.textContent = (formattedTwData[0]['tsid']).split(".")[1] + " (ppm)";
                ...
                cdaSaveBtn.id = "cda-btn-do";
                cdaSaveBtn.disabled = true;
                cdaSaveBtn.style.display = "none"; // Hides the button
                output20Div.appendChild(cdaSaveBtn);

                // Status Button
                const statusDiv = document.createElement("div");
                statusDiv.className = "status-do";
                output20Div.appendChild(statusDiv);

                // // Refresh Button
                // const buttonRefresh = document.createElement('button');
                // buttonRefresh.textContent = 'Refresh';
                // buttonRefresh.id = 'refresh-do-button';
                // buttonRefresh.className = 'fetch-btn';
                // output20Div.appendChild(buttonRefresh);
